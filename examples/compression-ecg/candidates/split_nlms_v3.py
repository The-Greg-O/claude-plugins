"""
split_nlms_v3 — Wider long-tap coverage for split-tap NLMS ECG compression.

Parent: split_nlms_v1 (best-of-45, 4.3133×).

Hypothesis: The champion's 3 split configs use 16 long taps centered at the
estimated beat period T (window T-8 to T+7). The QRS complex spans ~30-40
samples at 360 Hz; period jitter of ±10-14 samples puts the true R-peak near
the window edge. Progressively reallocating capacity from short (local) taps
to long (cross-beat) taps — while holding total_order constant at 32 — covers
the full QRS: (8,24) gives ±12 samples and (4,28) gives ±14 samples.
All new configs have the same per-step cost as (16,16) (32 total taps).
No regression: champion options (flags 0-29) and v1 split options are retained.

Best-of-50 = champion 30 + 20 split-NLMS (5 configs × 5 pack types).
Flags 0-29: champion (pred_idx=flag//5, pack_type=flag%5).
Flags 30-54: split-NLMS (split_idx=(flag-30)//5, pack_type=(flag-30)%5).
...
# (short_order, long_order, mu) — all new configs keep total_order ≤ 32
_SPLIT_CONFIGS = [
    (16, 16, 0.03),  # v1 config 0: balanced local/long, ±8 window
    (16, 16, 0.01),  # v1 config 1: balanced, slow adaptation
    (8, 16, 0.03),   # v1 config 2: less local, ±8 window
    (8, 24, 0.03),   # new: less local, ±12 window (wider QRS coverage)
    (4, 28, 0.03),   # new: minimal local, ±14 window (max cross-beat coverage)
]
...
def encode(blob):
    ...
    for c in range(n_chan):
        ...
        # Split-NLMS options: flags 30-54 (5 configs × 5 pack types)
