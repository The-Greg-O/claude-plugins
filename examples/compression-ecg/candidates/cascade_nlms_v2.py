"""
cascade_nlms_v2 -- two-stage cascade NLMS with order-1 stage-2 config.

Parent: rice_nlms_v1 (champion 4.3113x, best-of-30).
Lineage: cascade-nlms.

Builds on cascade_nlms_v1 (written but never evaluated):
- Probe on train/101.ecg NLMS-8 residuals: lag-1 autocorrelation = 0.807.
  A first-order NLMS (order=1, mu=0.50) directly models this AR(1) structure,
  converging to a[0]~0.807 within a few samples. Stage-2 residual variance
  ~= (1 - 0.807^2) * Var(r1) = 0.35 * Var(r1) -> ~65% variance reduction.
- Adds s2=(1, 0.50) as _CASCADE_S2_CONFIGS index 2 alongside v1's (2,0.30)/(4,0.20).
- Three new cascade pairs: (raw,NLMS1), (NLMS4,NLMS1), (NLMS8,NLMS1).

Best-of-65 = champion 30 + 7 cascade pairs x 5 pack types.
Champion options are a strict subset => no regression by construction.

Flag encoding:
  pred_idx = flag // 5  (0-5: champion raw/NLMS configs; 6-12: cascade pairs)
  pack_type = flag % 5  (0=varzz+bz2, 1=nibble+bz2, 2-4=Rice k=1/2/3)

For cascade flags (pred_idx >= 6), data layout:
  [uint8 s1_idx][uint8 s2_idx] then packed residuals
  s1_idx: 0=raw_diff, 1-5 = _NLMS_CONFIGS index+1
...
# Stage-2 configs; index 2 is new (order=1 directly targets lag-1 AR structure)
_CASCADE_S2_CONFIGS = [(2, 0.30), (4, 0.20), (1, 0.50)]

# Cascade pairs: (s1_idx, s2_idx)
# s1_idx 0=raw diff, 1=NLMS_CONFIGS[0]=(2,.20), 2=NLMS_CONFIGS[1]=(4,.10),
#        3=NLMS_CONFIGS[2]=(8,.05), ...
_CASCADE_PAIRS = [
    (0, 0),  # raw_diff + NLMS-2/mu=0.30   (from v1)
    (2, 0),  # NLMS-4   + NLMS-2/mu=0.30   (from v1)
    (3, 0),  # NLMS-8   + NLMS-2/mu=0.30   (from v1)
    (3, 1),  # NLMS-8   + NLMS-4/mu=0.20   (from v1)
    (0, 2),  # raw_diff + NLMS-1/mu=0.50   NEW: pure lag-1 removal
    (2, 2),  # NLMS-4   + NLMS-1/mu=0.50   NEW
    (3, 2),  # NLMS-8   + NLMS-1/mu=0.50   NEW
]


# ---------------------------------------------------------------------------
# Entropy coding helpers (identical to champion rice_nlms_v1)
# ---------------------------------------------------------------------------
...
def encode(blob):
    ...
    for c in range(n_chan):
        ...
        stage1 = [dc]  # 0=raw, 1..5=NLMS configs
        ...
        # Cascade options: stage-2 NLMS on stage-1 residuals.
        # Cache stage-2 outputs keyed by (s1_idx, s2_idx) to avoid recomputation.
        s2_cache = {}
        for pair_idx, (s1_idx, s2_idx) in enumerate(_CASCADE_PAIRS):
            key = (s1_idx, s2_idx)
            if key not in s2_cache:
                order2, mu2 = _CASCADE_S2_CONFIGS[s2_idx]
                s2_cache[key] = _nlms_fwd(stage1[s1_idx], order2, mu2)
            r2 = s2_cache[key]
...
def decode(blob):
    ...
    for _ in range(n_chan):
        ...
        if is_cascade:
            order2, mu2 = _CASCADE_S2_CONFIGS[s2_idx]
            vals = _nlms_inv(vals, order2, mu2)
            if s1_idx > 0:
                order1, mu1 = _NLMS_CONFIGS[s1_idx - 1]
                vals = _nlms_inv(vals, order1, mu1)
        else:
            if pred_idx > 0:
