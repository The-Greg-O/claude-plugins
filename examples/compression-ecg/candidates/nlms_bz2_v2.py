"""
nlms_bz2_v2 — best-of-8: raw_vz, raw_nb, + three NLMS configs each with varzz/nibble.

Extends champion nlms_bz2_v1 with two additional NLMS hyperparameter configs:
  order=2 mu=0.20  fast-adapt, short memory (better for highly irregular beats)
  order=4 mu=0.10  champion setting
  order=8 mu=0.05  slow-adapt, long memory (better for gradual-drift segments)

Best-of-8 per channel guarantees no regression vs champion; wider hyperparameter
coverage targets irregular-rhythm holdout records (e.g. 207 LBBB) where the
champion's single config leaves residuals on the table.

Losslessness: weight updates use the quantised residual (not the float prediction
error), so encoder and decoder NLMS states advance identically sample by sample.

Flag byte encoding (stored per channel):
  0 = raw varzz
  1 = raw nibble
  2 = nlms(2,0.20) varzz
  3 = nlms(2,0.20) nibble
  4 = nlms(4,0.10) varzz
  5 = nlms(4,0.10) nibble
  6 = nlms(8,0.05) varzz
  7 = nlms(8,0.05) nibble
...
_NLMS_CONFIGS = [(2, 0.20), (4, 0.10), (8, 0.05)]
