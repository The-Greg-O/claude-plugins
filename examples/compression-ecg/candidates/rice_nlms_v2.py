"""
rice_nlms_v2 — Add LZMA backend + extended Rice k range to champion.

Parent: rice_nlms_v1 (best-of-30: 6 predictor configs x {varzz+bz2, nibble+bz2, Rice-1/2/3}).

Hypothesis: (1) After NLMS removes heartbeat periodicity, LZMA's LZ77 sliding-window
may find shorter-range repeating patterns in residuals better than bz2's BWT on blocks
where periodic structure is absent. (2) Rice k=0 covers near-zero-mean residuals where
mean(abs) < 1; Rice k=4 covers heavier-tailed residuals (std ~10-16) e.g. raw delta
on irregular records. Best-of-48 = 6 configs x 8 options; regression-proof.

Flag encoding (8 options per predictor config):
  pred_idx = flag // 8   (0=raw, 1..5 = NLMS configs 0..4)
  pack_type = flag % 8
    0 = varzz+bz2
    1 = nibble+bz2
    2 = varzz+lzma
    3 = Rice-k0  (unary, best for near-zero residuals)
    4 = Rice-k1
    5 = Rice-k2
    6 = Rice-k3
    7 = Rice-k4  (best for heavier tails, std ~10-16)
...
_RICE_KS = [0, 1, 2, 3, 4]
_LZMA_PRESET = 4  # faster than default-6; small ratio loss (<1%) worth the ~4x speedup
...
def _rice_encode(zzag, k):
    ...
    if k == 0:
        # Special case: unary coding — no remainder bits
        total_bits = int(zzag64.sum()) + n
        total_bytes = (total_bits + 7) >> 3
        bits = np.zeros(total_bytes * 8, dtype=np.uint8)
        # position of the '1' stop bit for sample i is cumsum(zzag+1) - 1
        code_lens = zzag64 + 1
        starts = np.empty(n + 1, dtype=np.int64)
        starts[0] = 0
        np.cumsum(code_lens, out=starts[1:])
        stop_pos = (starts[:n] + zzag64).astype(np.int64)
        bits[stop_pos] = 1
        return struct.pack('<I', total_bits) + np.packbits(bits[:total_bytes * 8]).tobytes()
    q = zzag64 >> k
    r = zzag64 & ((np.uint64(1) << np.uint64(k)) - np.uint64(1))
    total_bits = int(q.sum()) + n * (1 + k)
    total_bytes = (total_bits + 7) >> 3
    bits = np.zeros(total_bytes * 8, dtype=np.uint8)
    code_lens = q + np.uint64(1 + k)
    ...
    for j in range(k):
        ...
        bits[rem_pos] = ((r >> np.uint64(k - 1 - j)) & np.uint64(1)).astype(np.uint8)
    return struct.pack('<I', total_bits) + np.packbits(bits[:total_bytes * 8]).tobytes()
...
def _get_options(zzag, pred_idx):
    """Return list of (size, flag, data) for all 8 encoding options."""
    flag_base = pred_idx * 8
    c_vz_bz2 = bz2.compress(_varzz_pack(zzag), 9)
    c_nb_bz2 = bz2.compress(_nibble_pack(zzag), 9)
    c_vz_lzma = lzma.compress(_varzz_pack(zzag), preset=_LZMA_PRESET)
    opts = [
        (len(c_vz_bz2),  flag_base + 0, c_vz_bz2),
        (len(c_nb_bz2),  flag_base + 1, c_nb_bz2),
        (len(c_vz_lzma), flag_base + 2, c_vz_lzma),
    ]
    for ri, k in enumerate(_RICE_KS):
        c_rice = _rice_encode(zzag, k)
        opts.append((len(c_rice), flag_base + 3 + ri, c_rice))
    return opts
...
def decode(blob):
    ...
    for _ in range(n_chan):
        ...
        if pack_type == 0:
            ...
            zzag = _varzz_unpack(lzma.decompress(data), n_samp)
        else:
            zzag = _rice_decode(data, n_samp, _RICE_KS[pack_type - 3])
