"""
split_nlms_v1 — NLMS with cross-beat split taps for ECG compression.

Parent: rice_nlms_v1 (best-of-30).

Hypothesis: Standard NLMS-32 uses consecutive taps at lags 1-32. ECG has beat
periodicity T (200-500 samples at 360 Hz). A split filter with short taps at
lags 1-16 (local trend) AND long taps at lags T to T+15 (cross-beat) predicts
QRS morphology from the previous beat, cutting QRS residuals from zzag~8 to
zzag~2 on regular rhythms. bz2 on smaller residuals compresses more. No onset
detection needed — NLMS learns the cross-beat coupling automatically. On
irregular rhythms the long weights converge to ~0, so performance matches
or exceeds NLMS-32.

Beat period T: estimated per channel via FFT autocorrelation of diff^2. Stored
as uint16 in channel data for deterministic decode.

Best-of-45 = champion 30 + 15 split-NLMS (3 configs × 5 pack types).
Flags 0-29: champion (pred_idx=flag//5, pack_type=flag%5).
Flags 30-44: split-NLMS (split_idx=(flag-30)//5, pack_type=(flag-30)%5).
Channel data for flags 30-44:
  [uint16 beat_period][compressed_residuals per pack_type]
"""
import bz2
import struct
...
def _split_fwd(diff_ch, short_order, long_start, long_order, mu):
    ...
    # Use incrementally updated index array to avoid per-iteration modulo on all_offsets
...
def _pack_compress(zzag, pack_type, k_idx=None):
    """Encode zzag with one of the 5 pack types. Returns bytes."""
    if pack_type == 0:
...
def _pack_decompress(data, n, pack_type):
    """Decode zzag from pack_type encoding."""
    if pack_type == 0:
...
def encode(blob):
    ...
    for c in range(n_chan):
        ...
        # Split-NLMS options: flags 30-44
        # Estimate beat period once per channel
