//! cpq05: cpq04 + 2x unroll of Myers outer loop.
//!
//! Current Myers loop: 69 instrs/iter (60 core + 9 overhead: ptr++, counter--, br_if).
//! Unrolling 2x: share one 9-instr overhead per 2 chars = 4.5 overhead/char saved.
//! Odd n handled by one pre-step before the pair loop.
//! Expected savings: ~6,500-7,000 fuel (~6%).
...
pub extern "C" fn solve(a_ptr: u32, a_len: u32, b_ptr: u32, b_len: u32) -> u32 {
    ...
    // Each pair shares one loop-overhead block (ptr+=2, counter--, br_if) instead of two.
    while bp < b_end {
        let c1 = unsafe { *bp };
