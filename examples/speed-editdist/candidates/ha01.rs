//! ha01: pt01 + high-bit (top-of-word) pattern alignment.
//!
//! KEY CORRECTION (measured, not inferred): the trimmed Myers loop length n is
//! NOT 1-4 (as the notebook claimed) — measured mean is n≈28.8, m≈30 over the
//! frozen vectors. The Myers INNER LOOP dominates fuel, so shaving ops/iter is
//! worth ~3,000 fuel per op.
//!
//! Idea: store the pattern in the TOP m bits of the u64 (offset = 64-m) so the
//! score bit is always bit 63. Score extraction becomes `ph >> 63` (a single
//! shift) instead of `(ph >> (m-1)) & 1` (shift + mask) — removing 2 AND ops
//! per Myers iteration. The low-bit garbage that the NOTs introduce below the
//! pattern region is provably harmless: in `(eq & pv) + pv`, the addend
//! `eq & pv` is zero below the pattern, so no carry can originate there and
//! propagate up into the valid region. Verified correct (masked AND unmasked)
//! on all 90 frozen + 5000 random pairs via an offline u64 model.
//!
//! Bonus: `pv = !0 << off` and `bd = 1 << off` work uniformly for m=1..=64, so
//! the `m==64` special-case branch disappears.
//!
//! New lineage: hi-align. Parent: pt01.
...
unsafe fn myers_step(
    ...
    let ph = *mv | !(xh | *pv);
    let mh = *pv & xh;
    // Pattern occupies the top m bits, so the scoring row is bit 63: a bare
    // `>> 63` yields 0/1 with no mask.
...
pub extern "C" fn solve(a_ptr: u32, a_len: u32, b_ptr: u32, b_len: u32) -> u32 {
    ...
    let mut pv: u64 = !0u64 << off; // top m bits set; m=64 -> all ones
    let mut mv: u64 = 0u64;
    let mut score: u64 = m as u64;
    let bd: u64 = 1u64 << off;      // boundary bit injected on the horizontal shift
