//! Myers/Hyyrö bit-parallel + word-at-a-time trim (hybrid03).
//! Two micro-optimizations over hybrid02:
//! 1. Combined score update: compute delta=(ph_bit - mh_bit) as i32, then
//!    one wrapping_add instead of separate += and -= — eliminates the
//!    intermediate score store/reload between the two statements.
//! 2. PEQ init uses a tracked bit variable (shift-accumulate) instead of
//!    enumerate() to avoid recomputing 1u64<<i each iteration.
...
pub extern "C" fn solve(a_ptr: u32, a_len: u32, b_ptr: u32, b_len: u32) -> u32 {
    ...
    // PEQ init: track bit via shift-accumulate instead of 1u64<<i each time.
    let mut bit = 1u64;
    for &c in a.iter() {
    ...
    for &c in b.iter() {
        ...
        let ph = mv | !(xh | pv);
        let mh = pv & xh;
        // Combined update: one wrapping_add instead of separate += and -=,
        // avoiding an intermediate store/reload of `score`.
